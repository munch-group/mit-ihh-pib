#!/usr/bin/env python3
"""
Fix chromosome X haplotype data to properly handle male hemizygosity.

Males should have only 1 haplotype in non-PAR regions, but the current
conversion treats all individuals as diploid (2 haplotypes each).

This script:
1. Infers sex from the VCF genotypes (males are homozygous across non-PAR X)
2. Removes duplicate haplotypes for males in non-PAR regions
3. Keeps both haplotypes for females and PAR regions
...
def infer_sex_from_vcf(vcf_path, sample_size=1000, verbose=True):
    ...
    if verbose:
        ...
        sys.stderr.write(f"Expected haplotypes: {males * 1 + females * 2}\n")

    return is_male, sample_ids

def fix_hap_file(hap_in_path, legend_in_path, hap_out_path, is_male, verbose=True):
    """
    Fix HAP file by removing duplicate male haplotypes in non-PAR regions.

    HAP format: space-separated haplotypes, two per diploid individual
    For males in non-PAR: keep only first haplotype, skip second
    """
    if verbose:
    ...
    with auto_open(hap_in_path, 'rt') as f_in, auto_open(hap_out_path, 'wt') as f_out:
        for line_no, line in enumerate(f_in):
            ...
            # Build output: for each sample, take appropriate haplotypes
            output_haps = []
            for sample_idx in range(n_samples):
                ...
                if is_par:
                    # PAR region: keep both haplotypes for everyone
                    if hap2_idx < len(haps):
                        hap2 = haps[hap2_idx]
                        output_haps.extend([hap1, hap2])
                    else:
                        output_haps.append(hap1)
                else:
                    # Non-PAR region
                    if is_male[sample_idx]:
                        # Male: keep only first haplotype
                        output_haps.append(hap1)
                    else:
                        # Female: keep both haplotypes
                        if hap2_idx < len(haps):
                            hap2 = haps[hap2_idx]
                            output_haps.extend([hap1, hap2])
                        else:
            ...
            if verbose and (line_no + 1) % 100000 == 0:
                ...
                sys.stderr.write(f"Processed {line_no+1} variants ({par_status}, {len(output_haps)} haps)...\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix chrX HAP file to handle male hemizygosity",
    ...
    parser.add_argument('--hap-out', required=True, help='Output HAP file (fixed)')
    ...
    if args.verbose:
        sys.stderr.write("="*60 + "\n")
        sys.stderr.write("Fix chrX haplotypes for male hemizygosity\n")
        sys.stderr.write("="*60 + "\n\n")
